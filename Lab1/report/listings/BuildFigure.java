...
public class BuildFigure extends JPanel {
    ...
    public void paint(Graphics g) {
        final BufferedImage bi = new BufferedImage(1000, 3000, 
        ...
        for(GraphElement element : elementList) {
            if(ElementShape.ELLIPSE.equals(element.getElementShape())) {
                Ellipse2D ellipse = new Ellipse2D.Double(start_with.x, 
					start_with.y, width, height);
                g2.drawString(element.getNode().text, 
					(float) (ellipse.getCenterX() - 
						element.getNode().text.length() * 3),
					(float) ellipse.getCenterY());
                g2.draw(ellipse);
                prev_elem_end.setLocation(ellipse.getMaxX(),
							ellipse.getMaxY() + 50);
                ...
                prev_elem_end = drawSquare(new Point(
									start_with.x, prev_elem_end.y),
					element.getNode());
    ...
    private Point drawSquare(Point drawPoint, ASTEntry node){
        ...
        g2.drawString(node.text, (float) (rectangle.getCenterX() 
						- node.text.length() * 3), (float) rectangle.getCenterY());
        g2.draw(rectangle);
        int x = (int)prevShape.getBounds().getCenterX();
        int y = (int)prevShape.getBounds().getMaxY();
    ...
    private void findUpper(Shape figure){
        ...
        for(int i = 0; i < index; i++){
            ...
            if(x == figure.getBounds().getX() && 
									(figure.getBounds().getY() - y <= 100))
                g2.drawLine(x + width / 2, y + height, 
					(int)figure.getBounds().getCenterX(), (int)figure.getBounds()
											.getMinY());
    ...
    private void drawBlock(int listIndex, int nodeIndex, Point drawPoint){
        ...
        for(int i = nodeIndex + 1; i < blockElems.size(); i++){
            if(ElementShape.SQUARE.equals(blockElems.get(i)
											.getElementShape())) {
                blockPoint = drawSquare(blockPoint, blockElems.get(i)
											.getNode());
                drawEndLoop(blocks.get(listIndex), blockElems.get(i).getNode(),
											blockPoint);
            }
            else if(ElementShape.DIAMOND.equals(blockElems.get(i)
											.getElementShape())) {
                drawPoint = drawDiamond(blockPoint, blockElems.get(i)
											.getNode());
                ...
                blocks.get(blocks.indexOf(block)).setDrawn(true);
                drawBlock(blocks.indexOf(block), block.getNodes()
									.indexOf(block.getStartsWith()), drawPoint);
            }
        }
    }
    ...
    private void drawEndLoop(Block block, ASTEntry node, Point startPoint){
        ...
        g2.drawLine(startPoint.x, startPoint.y, finalPoint.x - width,
									startPoint.y);
        startPoint.setLocation(finalPoint.x - width, startPoint.y);
        g2.drawLine(startPoint.x, startPoint.y, startPoint.x, finalPoint.y -
									height / 2);
